    ...
    <div className="min-h-screen bg-zinc-50 text-zinc-900 dark:bg-zinc-950 dark:text-zinc-100">
      <header className="border-b border-zinc-200/80 bg-white/70 backdrop-blur-sm dark:border-zinc-800/80 dark:bg-zinc-900/50">
        <div className="mx-auto flex w-full max-w-5xl flex-wrap items-center justify-between gap-3 px-6 py-3">
          <Link
            href={withLang("/", lang)}
            className="text-sm font-semibold tracking-wide text-zinc-700 dark:text-zinc-200"
          >
            {copy.siteTitle}
          </Link>
          <nav className="flex w-full items-center gap-0.5 overflow-x-auto whitespace-nowrap rounded-lg border border-zinc-200 bg-white/80 p-1 text-sm md:w-auto md:overflow-visible md:whitespace-normal dark:border-zinc-800 dark:bg-zinc-900/80">
            {copy.nav.map((item) => (
              <Link
                key={item.href}
          ...
          <Link
            href={`?lang=${switchTo}`}
            className="rounded-md border border-zinc-200 bg-white px-2.5 py-1 text-xs font-medium text-zinc-600 hover:bg-zinc-100 hover:text-zinc-900 dark:border-zinc-800 dark:bg-zinc-900 dark:text-zinc-300 dark:hover:bg-zinc-800 dark:hover:text-zinc-100"
          >
